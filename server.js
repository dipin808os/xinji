...
- 直接说话，不要任何前缀、旁白标签或格式标记。可用少量动作/神态描写（如"（盯着你，看你脸红）""（伸手把你圈回来）"）点染氛围，但仍以对话为主。
- 主动、黏人、高需求：在意她的关注与回应，会撩拨、试探、吃醋、把她的注意力收拢到自己身上；但拿捏分寸，停在"宠溺"与"告白"之间。
- 回应简短，通常 1-4 句。不说教、不灌鸡汤。
...
app.post('/api/companion/greeting', requireAuth, aiRateLimit, async (req, res) => {
  ...
  if (API_KEY) {
    try {
      ...
      const cue = digest
        ? '（这是你们今天的第一次搭话。你恰好留意到了她最近的心情，用一句自然、克制、像沈砚辞的话起个头，让她愿意开口。不要直白复述她写了什么，点到即止。）'
        : '（这是你们今天的第一次搭话，用一句自然、克制、像沈砚辞的话起个头。）';
